...
World *scr_checkworld(lua_State *L, int idx) {
	void **ud = luaL_checkudata(L, idx, CSSCRIPTS_MWORLD);
	luaL_argcheck(L, *ud != NULL, idx, "Invalid world");
	return (World *)*ud;
}

World *scr_toworld(lua_State *L, int idx) {
	void **ud = luaL_testudata(L, idx, CSSCRIPTS_MWORLD);
	return ud ? *ud : NULL;
}
...
void scr_pushworld(lua_State *L, World *world) {
	...
	lua_getfield(L, LUA_REGISTRYINDEX, CSSCRIPTS_RWORLDS);
	lua_getfield(L, -1, name);
	if(lua_isnil(L, -1)) {
		lua_pop(L, 1);
		lua_newuserdata(L, sizeof(World *));
		luaL_setmetatable(L, CSSCRIPTS_MWORLD);
...
void scr_clearworld(lua_State *L, World *world) {
	...
	lua_pushnil(L);
	lua_setfield(L, -3, World_GetName(world)); // CSSCRIPTS_RWORLDS[wname] = nil;
...
static int meta_setspawn(lua_State *L) {
	...
	if(!lua_isnil(L, 2)) svec = scr_checkfloatvector(L, 2);
	if(!lua_isnil(L, 3)) sang = scr_checkangle(L, 3);
...
int scr_libfunc(world)(lua_State *L) {
	lua_newtable(L);
	lua_setfield(L, LUA_REGISTRYINDEX, CSSCRIPTS_RWORLDS);
