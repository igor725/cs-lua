...
static int particle_create(lua_State *L) {
	...
	luaL_setmetatable(L, CSLUA_MPARTICLE);
	int top = lua_gettop(L);

	if(lua_checktabfield(L, 1, "uv", LUA_TTABLE)) {
			for(int i = 0; i < 4; i++) {
	...
	if(lua_checktabfieldud(L, 1, "tintcolor", CSLUA_MCOLOR))
		part->tintCol = *lua_tocolor3(L, -1);
	if(lua_checktabfield(L, 1, "framecount", LUA_TNUMBER))
		part->frameCount = (cs_byte)lua_tointeger(L, -1);
	if(lua_checktabfield(L, 1, "particlecnt", LUA_TNUMBER))
		part->particleCount = (cs_byte)lua_tointeger(L, -1);
	if(lua_checktabfield(L, 1, "size", LUA_TNUMBER))
		part->size = (cs_byte)lua_tointeger(L, -1);
	if(lua_checktabfield(L, 1, "sizevar", LUA_TNUMBER))
		part->sizeVariation = (cs_float)lua_tonumber(L, -1);
	if(lua_checktabfield(L, 1, "spread", LUA_TNUMBER))
		part->spread = (cs_float)lua_tonumber(L, -1);
	if(lua_checktabfield(L, 1, "speed", LUA_TNUMBER))
		part->speed = (cs_float)lua_tonumber(L, -1);
	if(lua_checktabfield(L, 1, "gravity", LUA_TNUMBER))
		part->gravity = (cs_float)lua_tonumber(L, -1);
	if(lua_checktabfield(L, 1, "lifetime", LUA_TNUMBER))
		part->baseLifetime = (cs_float)lua_tonumber(L, -1);
	if(lua_checktabfield(L, 1, "lifetimevar", LUA_TNUMBER))
		part->lifetimeVariation = (cs_float)lua_tonumber(L, -1);
	if(lua_checktabfield(L, 1, "colflags", LUA_TNUMBER))
		part->collideFlags = (cs_byte)lua_tointeger(L, -1);
	if(lua_checktabfield(L, 1, "fullbright", LUA_TBOOLEAN))
		part->fullBright = (cs_bool)lua_toboolean(L, -1);
