...
static Command *getstatecommand(lua_State *L, int idx) {
	...
	luaL_argcheck(L, Command_GetUserData(cmd) == Script_GetHandle(L), idx, errors[2]);
	return cmd;
}
...
static int cmd_add(lua_State *L) {
	...
	lua_getfield(L, LUA_REGISTRYINDEX, CSSCRIPTS_RCMDS);
	lua_pushvalue(L, 1);
	lua_pushvalue(L, 4);
	lua_settable(L, -3);
...
int scr_libfunc(command)(lua_State *L) {
	lua_newtable(L);
	lua_setfield(L, LUA_REGISTRYINDEX, CSSCRIPTS_RCMDS);
